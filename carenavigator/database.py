...
def get_dashboard_kpis():
    ...
    with get_connection() as conn:
        ...
        recent_ed_visits = row[0] if row else 0
        ...
        high_utilization = row[0] if row else 0
        ...
        current_encounters = row[0] if row else 0

        # ----------------------------------------------------
        # POTENTIALLY AVOIDABLE COMPLETED ENCOUNTERS
        # ----------------------------------------------------
        ...
        avoidable_ed = row[0] if row else 0

    return {
        "total_patients": int(total_patients or 0),
        "recent_ed_visits": int(recent_ed_visits or 0),
        "avoidable_ed": int(avoidable_ed or 0),
        "high_utilization": int(high_utilization or 0),
        "current_encounters": int(current_encounters or 0)
    }


# ============================================================
# ED AVOIDABILITY COUNTS
# ============================================================

def get_ed_avoidability_counts():
    """
    Count completed encounters by prediction.

    prediction = 1 -> Potentially Avoidable
    prediction = 0 -> Non-Avoidable
    """
    ...
    with get_connection() as conn:
        ...
        non_avoidable = row[0] if row else 0

    return {
        "avoidable": int(avoidable or 0),
        "non_avoidable": int(non_avoidable or 0)
    }
